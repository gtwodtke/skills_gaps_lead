...
################################################
##                                            ##
## PROGRAM NAME: 02_create_table_a2           ##
## AUTHOR: GW                                 ##
## DATE: 8/4/2021                             ##
## UPDATED: 3/21/2022                         ##
## DESCRIPTION:                               ##
...
for (i in 1:nmi) {
	...
	### OBSERVED GAPS ###
	m0.inc<-lm(natten_3~incq2,data=phdcn)
	m0.educ<-lm(natten_3~abovehs_b,data=phdcn)
	...
	boot.est<-foreach(h=1:nboot, .combine=cbind) %dopar% {
		...
		boot.m0.inc<-lm(natten_3~incq2,data=boot.phdcn)
		boot.m0.educ<-lm(natten_3~abovehs_b,data=boot.phdcn)
...
for (i in 1:nmi) {
	...
	idata<-phdcn[,c(vars.b.cm,vars.w1.cm,"natten_3","nlnbll_3","nlnbll_2","nlnbll_1","nsubage_3","incq2","abovehs_b")]
	...
	boot.est.cm<-foreach(h=1:nboot, .combine=cbind) %dopar% {
		...
		boot.idata<-boot.phdcn[,c(vars.b.cm,vars.w1.cm,"natten_3","nlnbll_3","nlnbll_2","nlnbll_1","nsubage_3","incq2","abovehs_b")]
...
sink("D:\\projects\\skill_gaps_lead\\programs\\appendix_a\\_LOGS\\02_create_table_a2_log.txt")
...
save(lm.gap.w1,lm.gap.w2,lm.gap.w3,lm.gap.cm,
     ...
     ob.gap,
     file="D:\\projects\\skill_gaps_lead\\data\\_TEMP\\appendix.a.attn.gap.est.RData")
